document.addEventListener("DOMContentLoaded", () => {
  ...
  // თავდაპირველი სტატისტიკის განახლება
  updateSubjectStats();

  // ფორმის submit ივენთი
  form.addEventListener("submit", async (e) => {
    e.preventDefault();

    // Loading state-ის დაყენება
    ...
    try {
      ...
      // Draft-ის გასუფთავება წარმატებული შენახვის შემდეგ
      removeDraft();
    ...
    try {
      ...
      channel.addEventListener('message', (e) => {
        if (e.data.type === 'dataSync') {
          console.log("BroadcastChannel სინქრონიზაცია:", e.data);
          updateSubjectStats();
        }
      });
    } catch (error) {
  ...
  // Global objects ინიციალიზაცია
  if (!window.studyHubSubjects) {
    window.studyHubSubjects = DataManager.getSubjects();
  }
